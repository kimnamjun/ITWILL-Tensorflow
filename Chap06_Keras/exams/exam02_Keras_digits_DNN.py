...
x_train, x_val, y_train, y_val = train_test_split(x_data, y_one_hot, test_size = 0.3)

# 3. keras model
model = Sequential() 

# 4. DNN model layer 구축
model.add(Dense(32, input_shape=(64,), activation='relu'))  # 모르면 안쓰는게 나은듯
...
model.save("keras_model_digit.h5")
print("save model")
